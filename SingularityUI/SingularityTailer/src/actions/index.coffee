...
actions = (fetchData=null) ->
  unless fetchData
    fetchData = (taskId, path, offset=undefined, length=0) ->
      ...
      $.ajax
        url: "#{ window.config.apiRoot }/sandbox/#{ taskId }/read?#{$.param({path, length, offset})}"

  initialize = (requestId, path, search, taskIds, apiRoot) ->
    (dispatch, getState) ->
      { viewMode, singularityApiRoot } = getState()
      ...
      groupPromises = taskIdGroups.map (taskIds, taskGroupId) ->
        ...
        Promise.all(taskInitPromises).then ->
  ...
  updateFilesizes = ->
    (dispatch, getState) ->
      { tasks, singularityApiRoot } = getState()
      for taskId of tasks
  ...
  taskGroupFetchNext = (taskGroupId) ->
    (dispatch, getState) ->
      {tasks, taskGroups, logRequestLength, maxLines, singularityApiRoot} = getState()
  ...
  toggleTaskLog = (taskId) ->
    (dispatch, getState) ->
      {search, path, tasks, viewMode, singularityApiRoot} = getState()
      if taskId of tasks
